...
class MuZeroConfig:
    def __init__(self):
        ...
        self.observation_shape = (
            3, 3, 3)  # Dimensions of the game observation, must be 3D (channel, height, width). For a 1D array, please reshape it to (1, 1, length of array)
...
class Game(AbstractGame):
    ...
    def human_to_action(self):
        ...
        while True:
            try:
                ...
                type = int(
                    input(
                        f"Enter the type (0:WAIT,1:GROW, 2:COMPLETE,3:SEED) to play for the player {self.to_play()}: "
                    )
                )
                if type == 3:
                    source = int(
                        input(
                            f"Enter the source (0-36) to play for the player {self.to_play()}: "
                        )
                    )
                if type != 0:
                    target = int(
                        input(
                            f"Enter the target (0-36) to play for the player {self.to_play()}: "
                        )
                    )
                if type == 0:
                    choice = 0
    ...
    def action_to_string(self, action_number):
        ...
        1 - 37 : grow 0-36
        38 - 74 : complete 0-36
        75 - 1444 : seed 0-36 0-36
        """

        if action_number == 0:
            return f"WAIT"
        if 1 <= action_number <= 37:
            target = action_number - 1
            return f"GROW {target}"
        elif 38 <= action_number <= 74:
            target = action_number - 38
            return f"COMPLETE {target}"
        elif action_number >= 75:
            source = (action_number - 75) % 37
            target = (action_number - 75) / 37
            return f"SEED {source} {target}"
